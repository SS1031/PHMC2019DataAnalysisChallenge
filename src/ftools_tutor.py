...
def download_data():
    if 'train_FD004.txt' not in os.listdir('../data/ftools'):
...
def new_labels2(data, labels):
    ...
    for engine_no_df in gb:
        ...
        r = randint(5, instances - 1)
        ct_ids.append(engine_no_df[1].iloc[r, :]['engine_no'])
        ct_cycles.append(engine_no_df[1].iloc[r, :]['time_in_cycles'])
        ct_labels.append(engine_no_df[1].iloc[r, :]['RUL'])

    ct = pd.DataFrame({'engine_no': ct_ids,
                       'cutoff_time': ct_cycles,
                       'RUL': ct_labels})
    ct = ct[['engine_no', 'cutoff_time', 'RUL']]
    ct.index = ct['engine_no']
    ct.index = ct.index.rename('index')
...
data_path = '../data/ftools/train_FD004.txt'

splits = 10
data = load_data(data_path)

data2 = data.copy()
data2['time'] = data2['time_in_cycles']
cutoff_times = make_cutoff_times(data)
cutoff_times2 = make_cutoff_times(data2)
...
fm2, features = ft.dfs(entityset=es2,
                       target_entity='engines',
                       agg_primitives=['last', 'max', 'min'],
                       trans_primitives=[],
                       ...
                       max_depth=3,
                       verbose=True)
feats = [f for f  in fm2.columns if 'time' not in f]
